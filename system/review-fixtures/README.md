...
> Kotlin 픽스처(`*.kt`)는 brain 고유 규칙(인자 개행·검증 위치·Controller 반환)의 탐지율을
...
```bash
# 골든셋을 리뷰 (전체 스코프)
/review:all system/review-fixtures/cases --full
#    → 리뷰가 방출한 findings 를 JSON 배열로 저장 (예: run_findings.json)
#      스키마: [{"area","category","severity","file":"cases/.../x.py:LN"}]

# 채점
python3 system/review-fixtures/eval.py --findings run_findings.json
...
python3 system/review-fixtures/eval_verifier.py --verdicts run_verdicts.json
...
```bash
python3 system/review-fixtures/eval_stability.py --a run1.json --b run2.json
...
## 언제 돌리나
- 위임 에이전트(`system/subagents/review-*.md`)·모델 변경 시 — **1)·2) 모두**
- 룰셋(RULESET_VERSION) 변경 시 — 회귀 확인 후 버전 bump
- 분기 1회 정기 (리뷰어 드리프트 점검)
- CI: `skills/review/**`·`system/subagents/review-*` 변경 PR 은
