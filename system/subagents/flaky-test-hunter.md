...
**원칙**:
- CI 로그에 없는 테스트는 추측하지 말 것. `gh` 가 없으면 "gh CLI 필요" 보고 후 종료.
- **재현은 검증 게이트다** (tautological 방지): Top 1 후보는 제시한 재현 명령을 가능하면 **직접 1회 실행**해 결과를 표에 병기 — `재현됨(N/M)` / `재현 안 됨` / `실행 불가(사유)`. 실행 못 했으면 후보를 "가설" 로 표시한다 — 재현 안 해본 재현 방법은 주장일 뿐이다.
