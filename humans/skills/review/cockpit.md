...
> @standards/CLAUDE.md · @standards/management/security.md
...
- [ ] `humans/skills/<category>/` 가 카테고리 단위로 `~/.claude/commands/<category>` 디렉토리 링크 가능
- [ ] `humans/subagents/` 가 `~/.claude/agents/` 로 링크 가능
- [ ] `scripts/global-install.sh` 와 `scripts/project-link.sh` 에 `[ -L ]` 또는 `readlink` 기반 중복 링크 방지 로직 존재

### §3. Skills 의 Standards 강제 준수 (15점)

각 `humans/skills/**/*.md` (단 `_template.md` 제외) 에 대해:

- [ ] YAML frontmatter 에 `follows-standards` 배열이 존재함. **경로 규약**: 설치 후 관점의 `standards/…` 로 기입하고, 검증 시 `core/standards/…` 로 resolve 하여 실제 파일 존재 확인 (이중 허용 금지)
- [ ] `enforcement: required` 또는 `recommended` 명시
- [ ] 본문 상단에 "Standards 준수 필수" 블록 존재 (`⚠️` + `@standards/...` 참조)
- [ ] `name: <category>:<slug>` 형식이 파일 경로(`humans/skills/<category>/<slug>.md`)와 일치
...
- [ ] `core/mcp-shared/servers.json` 이 유효한 JSON
...
- [ ] `core/mcp-shared/setup.sh` 에 `# Phase 1`, `# Phase 2`, `# Phase 3` 주석이 이 순서로 존재하고, Phase 3 앞에 `OPT_DRY_RUN` 분기 종료 로직 포함
- [ ] `core/mcp-shared/setup.sh` 가 `--dry-run` 옵션을 파싱하고 `OPT_DRY_RUN` 변수로 분기 처리
- [ ] `core/mcp-shared/clean.sh` 가 `--purge-env` 옵션을 파싱하고 `security delete-generic-password` 또는 `secret-tool` 포함
...
- [ ] `humans/subagents/` 에 최소 5개 에이전트 존재
...
**대상 범위 2종**:
- **(A) 진입점 스크립트**: `install.sh`, `scripts/*.sh` (lib 제외), `core/mcp-shared/setup.sh`, `core/mcp-shared/clean.sh`
- **(B) 라이브러리·훅**: `scripts/lib/*.sh`, `core/hooks/*.sh`
...
**§9b. 깨진 링크 (6점)**
- [ ] 모든 `@<path>` 참조가 실제 파일을 가리킴. **resolve 규칙**: `@standards/...` → `core/standards/...`, `@docs/...` → `docs/...`, `@skills/...` → `humans/skills/...`
...
- [ ] `core/CLAUDE.md` 와 `core/standards/CLAUDE.md` 의 한글 정책이 일치
...
# ============================================================
find humans/skills -type f -name '*.md' ! -name '_template.md' | while read -r f; do
...
# 3b) §3 — 본문 "Standards 준수 필수" 블록 (⚠️ + @standards/…) 존재 검사
find humans/skills -type f -name '*.md' ! -name '_template.md' | while read -r f; do
  body=$(awk '/^---$/{c++; next} c==2' "$f" | head -10)
  echo "$body" | grep -q '⚠️'               || { echo "MISSING ⚠️ block: $f"; continue; }
  echo "$body" | grep -q '@standards/'       || echo "MISSING @standards/ in ⚠️ block: $f"
done
...
done < <(find scripts core/hooks core/mcp-shared -type f -name '*.sh' -print0)
...
# ============================================================
find scripts core/hooks core/mcp-shared -type f -name '*.sh' | while read -r s; do
...
python3 <<'PY'
import json, re
d = json.load(open('core/mcp-shared/servers.json'))
...
python3 <<'PY'
import re
txt = open('core/mcp-shared/setup.sh').read()
...
grep -q -- '--purge-env' core/mcp-shared/clean.sh     || echo "clean.sh: MISSING --purge-env"
grep -qE 'secrets_delete|security delete-generic-password|secret-tool' core/mcp-shared/clean.sh \
...
#       @skills/...    → humans/skills/...
#       @subagents/... → humans/subagents/...
#       @docs/... · @core/... · @humans/... → 그대로
#       그 외 상대 참조 → 참조 원본 파일 기준 디렉토리 · core/standards/ · docs/ 3곳 시도
# ============================================================
grep -rnoE '@[a-zA-Z][a-zA-Z0-9/_.-]*\.md' --include='*.md' core/ humans/ docs/ \
  | while IFS=: read -r src line ref; do
      p="${ref#@}"
      case "$p" in
        standards/*)  real="core/$p" ;;
        skills/*|subagents/*) real="humans/$p" ;;
        docs/*|core/*|humans/*) real="$p" ;;
        *)
          # 컨텍스트 인식 fallback: 원본 파일 디렉토리 → standards → docs
          src_dir=$(dirname "$src")
          if   [ -f "$src_dir/$p"        ]; then real="$src_dir/$p"
          elif [ -f "core/standards/$p"  ]; then real="core/standards/$p"
          elif [ -f "docs/$p"            ]; then real="docs/$p"
          else real="core/standards/$p"  # 실패 시 표시용 경로
          fi
          ;;
      esac
...
git grep -nE '(ghp_[A-Za-z0-9]{20,}|xoxb-[A-Za-z0-9-]{20,}|sk-[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16}|glpat-[A-Za-z0-9_-]{20,})' \
  -- ':!*.example' ':!humans/skills/review/cockpit.md' \
  || echo "secrets: clean"
...
def walk(x):
    if isinstance(x, dict):
        ...
        for m in re.findall(r'core/hooks/([\w.-]+\.sh)', x): referenced.add(m)
        for m in re.findall(r'(?<!core/)hooks/([\w.-]+\.sh)', x): referenced.add(m)
walk(s.get('hooks', {}))
...
if command -v rg >/dev/null 2>&1; then
  rg -nP "$BANMAL_RE" core/ humans/ docs/ \
    --glob '*.md' --glob '!humans/skills/review/cockpit.md' | head -30
elif grep -rnP '' --include='*.md' core/ >/dev/null 2>&1; then
  grep -rnP "$BANMAL_RE" --include='*.md' \
    --exclude='cockpit.md' core/ humans/ docs/ 2>/dev/null | head -30
else
...
# ============================================================
find humans/subagents -type f -name '*.md' | while read -r f; do
  fm=$(awk '/^---$/{c++; if(c==2) exit; next} c==1' "$f")
