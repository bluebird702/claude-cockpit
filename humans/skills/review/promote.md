...
> ⚠️ **Standards 준수 필수** — @standards/CLAUDE.md · @standards/hard-won-conventions.md
> **사람 게이트 필수** — 이 스킬은 승격 후보를 **초안(draft)** 으로만 만든다.
> enforced 표준(`core/standards/**`)에 **자동 병합 금지**. 무인 자기수정 금지.
...
- `core/standards/**`·`hard-won-conventions.md`에 해당 규칙이 이미 있나
...
| 후보 성격 | 승격 대상 |
|-----------|----------|
| 일반화 가능한 반복 결함 | `core/standards/*` 규칙 강화 **+ 골든 픽스처 + expected.jsonl 줄** |
...
- **submodule 영향**: `core/standards/` 변경은 **전 프로젝트 배포**임을 명시
