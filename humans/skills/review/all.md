...
> **scale.tier** ★: @standards/engineering/reliability.md 의 티어 축. `prototype` 이면
...
> (`humans/subagents/review-*.md`, global-install 로 `~/.claude/agents` 링크)입니다 — 전문
...
> `humans/review-fixtures/README.md` §검증자 골든셋).
...
**launch-readiness 게이트 (scale.tier = hyperscale 일 때만 추가)** — @standards/engineering/reliability.md §5:
