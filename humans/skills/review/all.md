...
### 소스 (2개)
1. **base**: cockpit `@standards/*` (org baseline) — coding/testing/api/management
...
- source: base=cockpit@standards · overrides=[platform/account/.claude/rules/testing-strategy.md]
...
2. `@standards/…/X.md` **import**(baseline 참조 — 복사 금지)
3. **프로젝트 delta**(override·추가)만

baseline 문장을 재서술한 rule 이 보이면 drift 위험 → `/docs:migrate-standards` 로 정리(§강제).
...
- **각 파일 정독** ★: 디렉토리 목록·파일명만 보고 판단하지 말고, 스코프 내 파일을 실제로 **열어 읽는다**(특히 `import`·공유 가변 상태·쿼리·에러 처리·경로 기반 계층). 파일을 안 열고 `[]`(발견 없음)을 내는 관여 부족은 **리뷰 실패**다. (골든셋 실측: 정독 강제 시 탐지율 급상승 — 관여 2→9 tool-use, recall 0.43→1.0 의 핵심 레버)
...
> 합계 = **100%** (25+20+20+15+10+10). 이전 버전은 성능 15로 합이 105% 라
> 종합점수가 약 5% 부풀려졌었다 — 자기 리뷰에서 발견해 100% 로 정정.

- **종합 계산 (과락 캡 포함)**:
  1. `가중 = 보안×0.25 + 코드×0.20 + 아키텍처×0.20 + 테스트×0.15 + 성능×0.10 + 의존성×0.10`
...
FLOOR=80   # 과락 기준 · 통과선
assert abs(sum(W.values())-1.0)<1e-9, "가중치 합은 1.0 이어야 함"
...
print(json.dumps({"overall":overall,"weighted":weighted,"area":area,
...
> 가중 평균: 보안(25%)+코드(20%)+아키텍처(20%)+테스트(15%)+성능(10%)+의존성(10%) = 100%
