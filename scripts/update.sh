...
if [ -L "$dst" ] && [ "$(readlink "$dst")" = "$LOCAL_DIR/settings.json" ]; then
  ...
  # Claude Code 가 atomic write(임시파일→rename)로 심링크를 실파일로 대체한 드리프트 상태.
  # 실파일에는 런타임 상태(플러그인 토글·인증 캐시)가 있으므로 삭제하지 않고,
  # cockpit baseline(렌더본)을 딥머지로 덮어씀 — 렌더본 키가 우선, 런타임 전용 키는 보존.
...
for cat_dir in "$ROOT_DIR"/skills/*/; do
  ...
  if [ -L "$dst" ]; then
    ...
    log_warn "  ~ commands/$cat_name 실제 디렉토리 — ./install.sh --force 로 정리 필요"
  else
    ln -s "${cat_dir%/}" "$dst"
    log_ok "  + commands/$cat_name (신규 카테고리)"
  fi
done
...
# MEMORY.md 인덱스에 누락된 시드 라인 보강 (기존 라인은 건드리지 않음)
